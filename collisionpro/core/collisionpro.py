...
class CollisionPro:
    ...
    def run_episode(self):
        ...
        while True:
            action = None
            if self.controller:
                action = self.controller.get_action(self.env)

            if new_state is not None:
                episode["samples"].append({"state": new_state, "reward": reward})

            new_state, reward, terminated, truncated, info = self.env.step(action)

            if (terminated or truncated) and new_state is not None:
                episode["samples"].append({"state": new_state, "reward": reward})
                break
    ...
    def create_td_samples(self, episode):
        ...
        Args:
            episode: A dictionary containing processed episode data generated by the `run_episode` function.
